...
DWORD ProcessEventAddress = BaseProcessEventAddress - 0xe8;//0x00484190; //(dllBase + BaseProcessEventAddress);
...
wchar_t* charToWChar(char* s)
{
	...
	wchar_t* bufPtr = (wchar_t*)VirtualAlloc(0,32, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	//write to memory
...
void ImGuiWeapon(UHeroEquipment*_Weap)
{
	...
	ImGui::Text("InternalName : %s", _Weap->ObjectArchetype->GetName());



	ImGui::Text("ItemAddr          :  0x%p", _Weap);
	...
	if (_Weap->UserEquipmentName.Data)
	{
		...
		if (ImGui::Button("test", ButtonProperty))
		{
			TArray<wchar_t> a;
			a.Add('a');
			a.Add('%p');
			a.Add(' ');
			_Weap->UserEquipmentName.Data = a.Data;
			_Weap->UserEquipmentName.Count = 25;
			_Weap->UserEquipmentName.Max = 25;
		}
...
void ImguiMenuItemModding()
{
	...
	if (ImGui::CollapsingHeader("Item box", ImGuiTreeNodeFlags_None))
	{
		...
		{
			int* t = nullptr;
			heroManager->IsItemBoxFull(0, 0, t);
...
void PostRender(UCanvas* canvas)
{
	...
	//if (controller && !doonce)
	//{
	///*	doonce = !doonce;
	//	for (auto a : GetAllInstanceOf(UDunDefAchievementManager::StaticClass()))
	//	{
	//		auto o = (UDunDefAchievementManager*)a;
	//		for (int i = 0; i < o->AchievementEntries.Count; i++)
	//			((ADunDefPlayerController*)controller)->ClientDoUnlockAchievment(o->AchievementEntries.Data[i].AchievementId);
	//	}*/
	//	
	//	/*for (auto a : GetAllInstanceOf(ADunDefAchievementTrophy::StaticClass()))
	//	{
	//		auto o = (ADunDefAchievementTrophy*)a;
	//		if(o->bAchievementUnlocked == 0)
	//		o->SetAchievementUnlocked(1);
	//	}*/
	//	
	//}


	for (int i = 0; i < heroManager->CampaignLevelEntries.Count; i++)
	{

		heroManager->CampaignLevelEntries.Data[i].AlwaysUnlocked = 1;
		heroManager->CampaignLevelEntries.Data[i].IsRestrictedPremiumLevel = 0;
...
void __fastcall HookedPE(UObject* pObject, void* edx, UFunction* pFunction, void* pParms, void* pResult)
{
	...
	std::cout << "FuncName : "<< std::setw(100) << std::left << szName << " Object : " << objectName << "\n";
	...
	if (bAutoKill) {
		...
		if (t.find("DunDefTower") != std::string::npos && t.find("Tick") != std::string::npos)
		{
			...
			if (controller && controller->Pawn && e->TargetingTeam == 2)
			{
				...
				FVector tempVec = FVector();
				FTraceHitInfo tempHit = FTraceHitInfo();
				e->eventTakeDamage(e->MaxHealth, NULL, tempVec, tempVec, NULL, tempHit, NULL);
			}
		}
	}
...
BOOL APIENTRY OnAttach(HMODULE hModule)
{
	...
	//tmpfile.open("D:\\Logging\\DDItems.txt");
	//for (auto v : GetAllInstanceOf(UHeroEquipment::StaticClass()))
	//{
	//	tmpfile << v->GetName() <<"\n";
	//}
	//tmpfile.close();
	//ClientDoUnlockAchievment
